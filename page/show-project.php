...
if ( isset($_POST['updateProjectManager']) AND !isset($_POST['deleteit']) ) {
	if ( 'dataset' == $_POST['updateProjectManager'] ) {
		...
		if ( '' == $_POST['dataset_id'] ) {
			...
			$del_image = isset( $_POST['del_image'] ) ? true : false;
			$return_message = $projectmanager->editDataset( $_POST['name'], $_POST['grp_id'], $_POST['dataset_id'], $_POST['form_field'], $del_image, $_POST['image_file'] );
		}
...
?>
<div class="wrap">
	<?php if ( 1 < $projectmanager->getNumProjects()) $projectmanager->printBreadcrumb( $project_id, $project_title ) ?>
	
	<h2><?php echo $project_title ?></h2>
	
	<?php $projectmanager->printSearchForm( 'float: right; margin-bottom: 1em;' ); ?>
	<p>
		<a href="edit.php?page=projectmanager/page/edit-project.php&amp;id=<?php echo $project_id ?>"><?php _e( 'Edit Project Title', 'projectmanager' ) ?></a> &middot; <a href="edit.php?page=projectmanager/page/settings.php&amp;project_id=<?php echo $project_id ?>"><?php _e( 'Settings', 'projectmanager' ) ?></a> &middot;
	...
	<table class="widefat">
		...
		if ( $dataset_list ) :
			foreach ( $dataset_list AS $dataset ) :
				...
				$group = ( 0 != $dataset->grp_id ) ? $projectmanager->getGroups( $dataset->grp_id ) : 'Keine';
			?>
