			...
			<?php elseif ( 'date' == $form_field->type ) : ?>
				...
				<select size="1" name="form_field[<?php echo $form_field->id ?>][year]">
					...
					<?php for ( $year = 1970; $year <= date('Y')+10; $year++ ) : ?>
						<option value="<?php echo $year ?>"<?php selected ( $year, intval(substr($meta_data[$form_field->id], 0, 4)) ); ?>><?php echo $year ?></option>
					<?php endfor; ?>
				</select>
...
<input type="hidden" name="dataset_id" value="<?php echo intval($dataset->id) ?>" />
<input type="hidden" name="user_id" value="<?php echo intval($dataset->user_id) ?>" />
